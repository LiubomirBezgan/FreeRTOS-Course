...
TaskProfiler Timer1Profiler;
TaskProfiler Timer2Profiler;

osThreadId_t red_id;
osThreadId_t blue_id;
osThreadId_t green_id;
osThreadId_t orange_id;

osTimerId_t timer_1_id, timer_2_id;

osEventFlagsId_t EventFlag_LED;
const osEventFlagsAttr_t EventFlagAttribs_LED = {
		.name = "LED_Events"
};
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	EventFlag_LED = osEventFlagsNew(&EventFlagAttribs_LED);
...
void vGreenLedControllerTask(void * pvParameters)
{
	...
	{
		GreenTaskProfiler++;
		osEventFlagsWait(EventFlag_LED, 0x04, osFlagsWaitAny, osWaitForever);
...
//		osEventFlagsSet(EventFlag_LED, 0x01);
		osEventFlagsSet(EventFlag_LED, 0x02);
		osEventFlagsSet(EventFlag_LED, 0x04);
		osDelay(100);
	}
}
