...
{
	BaseType_t qStatus;
	// Enter the blocked state for 200 ms for space to become available in the queue each time the queue is full
...
void ReceiverTask (void *pvParameters)
{
	...
	while(1)
	{
		...
		if (pdPASS == qStatus)
		{
			if (humidity_sensor == xReceivedStructure.sDataSource)
			{
